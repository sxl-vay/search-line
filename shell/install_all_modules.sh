...
echo " 开始安装Search-Line项目的所有Maven模块到本地仓库... "
...
# 首先安装根目录的pom.xml
echo "${YELLOW}安装根目录的pom.xml... "
cd "${ROOT_DIR}"
if mvn clean install -N -DskipTests > "${LOG_DIR}/root.log" 2>&1; then
    echo "✓ 根目录pom.xml安装成功 "
    ((success_count++))
else
    echo "${RED}✗ 根目录pom.xml安装失败 "
    ((failed_count++))
    failed_modules="${failed_modules}\n- 根目录"
fi

# 查找所有包含pom.xml的目录
echo "${YELLOW}查找所有包含pom.xml的模块... "
...
for pom_file in $pom_dirs; do
    ...
    echo "${YELLOW}安装模块: ${relative_path} "
    
    # 切换到模块目录并执行mvn install
    cd "${module_dir}"
    if mvn clean install -DskipTests > "${log_file}" 2>&1; then
        echo " ✓ 模块 ${relative_path} 安装成功 "
        ((success_count++))
    else
        echo "${RED}✗ 模块 ${relative_path} 安装失败 "
        ((failed_count++))
        failed_modules="${failed_modules}\n- ${relative_path}"
    fi
...
# 输出安装结果摘要
echo "\n === 安装完成 === "
echo "${YELLOW}总耗时: ${minutes}分${seconds}秒 "
echo " 成功安装: ${success_count} 个模块 "

if [ ${failed_count} -gt 0 ]; then
    echo "${RED}安装失败: ${failed_count} 个模块 "
    echo "${RED}失败的模块:${failed_modules} "
    echo "${YELLOW}查看日志目录 ${LOG_DIR} 获取详细错误信息 "
    exit 1
else
    echo " 所有模块安装成功! "
    exit 0
